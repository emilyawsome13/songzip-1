...
def _disable_blocked_ytmusic_provider(downloader_settings):
    """
    Drop youtube-music from the active provider list when the probe fails.

    ### Arguments
    - downloader_settings: mutable downloader settings

    ### Returns
    - `True` if youtube-music was removed
    """

    audio_providers = list(downloader_settings["audio_providers"])
    if "youtube-music" not in audio_providers:
        return False
    ...
    fallback_providers = [
        provider for provider in audio_providers if provider != "youtube-music"
    ]
    if len(fallback_providers) == 0:
        fallback_providers = ["youtube"]

    downloader_settings["audio_providers"] = fallback_providers
    logger.warning(
        "YouTube Music is currently unavailable from this network. "
        "Continuing with fallback audio providers: %s",
...
def entry_point():
    ...
    # If YouTube Music is unavailable, prefer the remaining providers instead of
    # blocking the whole application startup.
    _disable_blocked_ytmusic_provider(downloader_settings)
