...
class Album(SongList):
    ...
    def get_metadata(url: str) -> Tuple[Dict[str, Any], List[Song]]:
        ...
        tracks = list(album_response["items"])

        # Get all tracks from album
        while album_response.get("next"):
        ...
        album_copyrights = album_metadata.get("copyrights") or []
        album_images = album_metadata.get("images") or []
        ...
        for track in tracks:
            ...
            song = Song.from_missing_data(
                ...
                cover_url=(
                    max(album_images, key=lambda i: i["width"] * i["height"])["url"]
                    if album_images
                    else None
                ),
                copyright_text=(
