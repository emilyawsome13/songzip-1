...
def get_spotdl_path() -> Path:
    ...
    env_override = os.environ.get("SPOTDL_DATA_DIR") or os.environ.get("SPOTDL_DIR")
    if env_override:
    ...
    if platform.system() == "Linux":
        ...
        xdg_config_path = home_dir / ".config" / "spotdl"

        # Define the old path (~/.spotdl) for backward compatibility
        old_spotdl_path = home_dir / ".spotdl"
...
WEB_OPTIONS: WebOptions = {
    ...
    "keep_sessions": False,
    "bundle_flatten": False,
    "bundle_compression": "deflate",
    "forced_format": None,
    "forced_output": None,
