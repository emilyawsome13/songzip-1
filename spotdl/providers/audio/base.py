...
class AudioProvider:
    ...
    def search_candidates(
        self, song: Song, only_verified: bool = False, limit: int = 5
    ) -> List[str]:
        """
        Search for a song and return multiple high-confidence candidate URLs.

        ### Arguments
        - song: The song to search for.
        - only_verified: Whether to restrict results to verified entries.
        - limit: Maximum number of candidate URLs to return.

        ### Returns
        - A list of candidate URLs ordered from best to worst.
        """
        ...
        collected_results: Dict[Result, float] = {}
        candidate_urls: List[str] = []
        fallback_results: List[Result] = []

        # Search by ISRC first when available.
        if song.isrc and self.SUPPORTS_ISRC and not self.search_query:
            isrc_results = self.get_results(song.isrc)
            ...
            if len(isrc_results) > 0:
                for result, score in sorted(
                    order_results(isrc_results, song, self.search_query).items(),
                    key=lambda item: item[1],
                    reverse=True,
                ):
                    if result.url not in candidate_urls:
        ...
        for options in self.GET_RESULTS_OPTS:
            ...
            if self.filter_results:
                ...
                new_results = {
                    result: float(max(100 - index, 1))
                    for index, result in enumerate(search_results)
                }

            if not new_results:
                for result in search_results[:limit]:
                    if result not in fallback_results:
                        fallback_results.append(result)

            collected_results.update(new_results)

        for result, _score in sorted(
            collected_results.items(),
            key=lambda item: item[1],
            reverse=True,
        ):
            if result.url not in candidate_urls:
    ...
    def get_best_result(self, results: Dict[Result, float]) -> Tuple[Result, float]:
        ...
        if len(best_results) > 1:
            ...
            for best_result in best_results:
                if best_result[0].views is not None:
